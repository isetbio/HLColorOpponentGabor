...
contrastArr = [0.5 0.6 0.7]%0:0.01:.14;
...
params.color_val = colorInd;          % 1 = s_iso, 2 = L-M, 3 = LMS, 4 = L-M
...
params.nSteps = 186;%666;          % total length of movie in millseconds
...
load('gaborData2/sensor_cv_3_cont_2000.mat','sensor')
% sensor = sensorSet(sensor,'photons',0);
% sensor = sensorSet(sensor,'photon rate',0);
% sensor = sensorSet(sensor,'volts',0);
...
for t = 1 : params.nSteps
    ...
    params.ph = 2*pi*(t-1)/(params.nSteps/1); % one period over nSteps
...
savestr2 = ['gaborData2/pooledData_long_cv_' num2str(params.color_val) '_Cont_' sprintf('%0.4d',10000*params.contrast) '.mat'];
save(savestr2);


clear sensor scene oi display params pooledData os
